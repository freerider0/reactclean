...
export const ConstraintToolbar: React.FC<ConstraintToolbarProps> = ({
  ...
  return (
    <div className={`absolute ${topPosition} right-4 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden`}>
      {/* Header */}
      <div
      ...
      {isExpanded && (
        <div className="p-4 max-h-[600px] overflow-y-auto">
          ...
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Add Constraint</h3>

            {/* Single Edge Constraints */}
            <div className="mb-3">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Edge Constraints (select 1 edge) or Diagonal</p>
            ...
            <div className="mb-3">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Two-Edge Constraints (select 2 edges)</p>
              <div className="grid grid-cols-3 gap-1">
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded ${
                    canAddTwoEdgeConstraint
                      ? 'bg-purple-500 dark:bg-purple-600 text-white hover:bg-purple-600 dark:hover:bg-purple-500'
                      : 'bg-gray-200 dark:bg-gray-700 text-gray-400 dark:text-gray-500 cursor-not-allowed'
                  }`}
            ...
            {diagonalConstraintMode && (
              <div className="mb-3">
                <div className="text-xs text-gray-600 dark:text-gray-300 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded p-2">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Active Constraints</h3>
              <span className="text-xs text-gray-500 dark:text-gray-400">{room.constraints.length} total</span>
            </div>

            {room.constraints.length === 0 ? (
              <p className="text-xs text-gray-400 dark:text-gray-500 italic">No constraints yet</p>
            ) : (
                ...
                {room.constraints.map((constraint) => (
                  <div
                    ...
                    className={`flex items-center justify-between px-2 py-1 rounded text-xs ${
                      constraint.enabled ? 'bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600'
                    }`}
                  >
                    <div className="flex items-center gap-2 flex-1">
                      <input
                        ...
                        className="w-3 h-3"
                      />
                      <span className={constraint.enabled ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}>
                        <span className="font-medium">{getConstraintTypeName(constraint.type)}</span>
                        {' '}
                        <span className="text-gray-500 dark:text-gray-400">{getConstraintDescription(constraint)}</span>
                      </span>
                    </div>
                    <button
